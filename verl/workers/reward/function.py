...
# Define callable date type : 两个字符串参数（生成的响应 response_str 和参考答案 ground_truth）返回一个符合 RewardScore 结构的字典
# ScoreFunction = Callable[[str, str], RewardScore]
# Adaptation
ScoreFunction = Callable[[str, str, float, Optional[int], Optional[str]], RewardScore]
...
class FunctionRewardManager:
    ...
    def __post_init__(self):
        ...
        # 使用getattr获取评分函数对象,使用functools.partial创建一个新的函数 
    ...
    def __call__(self, data: DataProto, step: Optional[int] = None, tool: bool = False) -> Tuple[torch.Tensor, Dict[str, List[float]]]:
        # DataProto 结构里应该有 batch["responses"] generated by model,Token ID, 
        ...
        for i in range(len(data)):
            ...
            if tool:
                QAid = data_item.non_tensor_batch["idx"]
                score = self.score_fn(response_str, ground_truth, step=step, QAid=QAid) # calling reward function
            else:
